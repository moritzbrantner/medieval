...
const [camera, scene, gpu, shader, browserBattle, architecture] = await Promise.all([
...
test("browser picking uses the same renderer-owned world projection as visible soldiers", () => {
  ...
  assert.doesNotMatch(browserBattle, /fn sanitized_zoom\(/);
});
